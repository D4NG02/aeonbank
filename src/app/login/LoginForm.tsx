...
const LoginForm = () => {
  ...
  const [isLogin, setIsLogin] = useState<LoginType>({ success: false, message: "Not login" });
  const [step, setStep] = useState<"username" | 'secure' | "submit">("username");
  ...
  return (
    <>
      {/* Submit username */}
      {step == "username" ? <Form onSubmit={onSubmit}>
        <Form.Input
          type="text"
          name="username"
      ...
      {/* Login */}
      {step == 'submit' ? <Form onSubmit={onLogin}>
        <span className="w-full">Your secure work: {secureWord}</span>
        {isLogin.success ? <span className="mr-auto px-2 py-1 rounded-lg bg-green-300">{isLogin.message}</span> : <></>}
